                        ...
                        <form action="" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input  placeholder="注意字数限制,超额的会用...代替" class="form-control"
                                      value="{{$list->name}}" name="name" id="name" required="required" type="text">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control m-b"  id="type" name="type" required="required">
                                        <option value="{{$list->type}}">请选择</option>
                                        <option value="click" @if($list->type=='click') selected @endif>点击推事件</option>
                                        <option value="view"  @if($list->type=='view') selected @endif>跳转URL</option>
                                        <option value="scancode_push"@if($list->type=='scancode_push') selected @endif>扫码推事件</option>
                                        <option value="scancode_waitmsg"  @if($list->type=='scancode_waitmsg') selected @endif>扫码推事件且弹出“消息接收中”提示框</option>
                                        <option value="pic_sysphoto"  @if($list->type=='pic_sysphoto') selected @endif>弹出系统拍照发图</option>
                                        <option value="pic_photo_or_album"  @if($list->type=='pic_photo_or_album') selected @endif>弹出拍照或者相册发图</option>
                                        <option value="pic_weixin"  @if($list->type=='pic_weixin') selected @endif>弹出微信相册发图器</option>
                                        <option value="location_select"  @if($list->type=='location_select') selected @endif>弹出地理位置选择器</option>
                                    </select>
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <input  placeholder="例如:https://isv.umxnt.com/admin/alipayopen" class="form-control"
                                        value="{{$list->url}}"  name="url" id="url" required="required" type="text">
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                <label>状态</label>
                            ...
                            <input type="hidden" id="pid" value="{{$list->pid}}">
                            <div class="hr-line-dashed"></div>
                            <div>
...
//                $("#url").focus();
//            }
            else if(($("#type").val()=='view')&&(!checkurl($("#url").val()))){
                layer.msg('请填写正确的url!');
                $("#url").focus();
            }else{

                $.post("{{route('WechatMenuEditpost')}}",
                        {
                            _token: '{{csrf_token()}}',
                        ...
                        function (result) {
                            if (result.success==1) {
                                if(result.pid!=0)
                                    window.location.href = "{{route('WxAppMenuSubList',['id'=>$list->pid])}}";
                                else
