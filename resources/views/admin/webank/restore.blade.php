...
@section('restore')
    ...
    <script type="text/javascript"> {{--modified by Neomor 彻底删除微众银行分店--}}
        function del(id) {
            //询问框
            // alert(id);
            layer.confirm('确定要彻底删除', {
                ...
                $.post("{{route('webankDelete')}}", {_token: "{{csrf_token()}}", id: id},  {{--modified by Neomor 彻底删除微众银行分店--}}
                    function (data) {
                        if(data.success){
                            window.location.href = "{{route('webankstoreback')}}"; {{--modified by Neomor 彻底删除微众银行分店--}}
                        }else{
