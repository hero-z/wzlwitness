            ...
            <div class="doc-example">
                <select data-am-selected="{searchBox: 1,maxHeight: 200}" style="display: none;" name="merchine" required>
                    ...
                    @foreach($unionpay as $v)
                        ...
                        @endif
                    @endforeach
                    @foreach($webank as $v)  {{--//add by Neomor 支持微众银行--}}
                        @if($v->name)  {{--//add by Neomor 支持微众银行--}}
                            <option value="{{$v->store_id}}*{{$v->name}}">{{$v->name}}(微众银行)</option>  {{--//add by Neomor 支持微众银行--}}
                        @endif  {{--//add by Neomor 支持微众银行--}}
                    @endforeach  {{--//add by Neomor 支持微众银行--}}
                </select>
            </div>
