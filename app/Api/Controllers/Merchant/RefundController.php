...
class RefundController extends \App\Api\Controllers\Merchant\BaseController
{
    //商户查询接口
...
//        $app_auth_token = AlipayShopLists::where('store_id', $store_id)->first()->app_auth_token;
        $app_auth_token = AlipayAppOauthUsers::where('store_id', $store_id)->first();
        Log::info($app_auth_token);
        return json_encode($this->AlipayTradeRefund($tradeNo,$refundAmount,$app_auth_token));
//        return ['code'=>'1000',
//            'msg'=>$store_id
//            ];
    }
    ...
    public function AlipayTradeRefund($out_trade_no, $refund_amount,$app_auth_token)
    {
        $ao = new AlipayOpenController();
        $aop = $ao->AopClient();
        $aop->method = 'alipay.trade.refund';
        $requests = new AlipayTradeRefundRequest();
        $out_request_no = time() . rand(1000000, 9999999);
        $requests->setBizContent("{" .
            "\"out_trade_no\":\"" . $out_trade_no . "\"," .
            "\"refund_amount\":\"" . $refund_amount . "\"," .
            "\"out_request_no\":\"" . $out_request_no . "\"" .
            "}");
        Log::info($requests->getBizContent());
        Log::info($requests->getApiMethodName());
        Log::info($app_auth_token);
        $result = $aop->execute($requests, '', $app_auth_token);
        return $result;
    }
