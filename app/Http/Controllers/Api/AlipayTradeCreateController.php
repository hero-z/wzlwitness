...
use App\Http\Controllers\alipass\alipassController;
use App\Merchant;
...
class AlipayTradeCreateController extends BaseController
{
    ...
    public function AlipayOqrCreate(Request $request)
    {
        ...
        $user = $request->session()->get('user_data');
        //检测用户是否有卡券
        $pass=PassUser::where("user_id",$user[0]->user_id)->first();
        if(!empty($pass)){
            $channel_id=$pass['channel_id'];
            $serial_number=$pass['serial_number'];
            //调用卡券接口
            $alipass=new alipassController();
            $useAlipass=$alipass->updatecardTemplete($serial_number,$channel_id,$user[0]->user_id);
            if($useAlipass){
                Log::info($useAlipass);
            }
        }
