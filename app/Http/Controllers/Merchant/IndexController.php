...
class IndexController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $webankcode=$this->webankcode("801");//add by Neomor
        $wwebankcode=$this->webankcode("802");//add by Neomor
        //扫码枪
         $moali=$this->moali();
         $msali=$this->msali();
        ...
        $mawebank=$this->webankcode("803");
        $mwwebank=$this->webankcode("804");
        //现金
        $cash=$this->cash();
        //支付宝
        $aliCode = $oalicode+$salicode+$palicode+$pufacode+$webankcode;
        $aliScan = $moali+$msali+$mpali+$mapufa+$mawebank;
        //微信
        $wxCode = $wxcode+$pweixin+$wpufacode+$wwebankcode;
        $wxScan = $mweixin+$mpweixin+$mwpufa+$mwwebank;
        //其他
        $otherCode = $pjd+$pbest+$unionpaycode;
        $otherScan = $mpjd+$munionpay;

        $code = $aliCode+$wxCode+$otherCode;//modified by Neomor
        $scan = $aliScan+ $wxScan +$otherScan ; //modified by Neomor
        $total =$code+$scan+$cash;
        if($total!=0){
            $a =round(($aliCode+$aliScan)/$total,5)*100; //modified by Neomor
            $b =round(($wxCode+$wxScan)/$total,5)*100;//modified by Neomor
...
//银联二维码
  ...
  //浦发二维码
    protected function pufacode($type){
        ...
        //微众银行二维码   add by Neomor
        protected function webankcode($type){
        $array=[$type];
        //判断登陆类型;
        $a=DB::table("merchant_shops")
            ->join('we_bank_stores', 'merchant_shops.store_id', '=', 'we_bank_stores.store_id')
            ->where('merchant_shops.merchant_id', auth()->guard('merchant')->user()->id)
            ->select("merchant_shops.store_id","we_bank_stores.pid","we_bank_stores.id")
            ->first();
        ...
        if($a){
            if($a->pid==0&&$b->type==0){
                //分店store_id
                $data[]=$a->store_id;
                $c=DB::table("we_bank_stores")->where("pid",$a->id)->get();
                if($c){
                    foreach($c as $v){
                        $data[]=$v->store_id;
                    }
