...
use App\Http\Controllers\WeBank\WebankPayController;
use App\Http\Controllers\WeBank\WebankBaseController;
...
//                        "    \"store_id\":\"" . $store->store_id . "\"," .
            ...
            if ($MerchantPayWay->alipay == "webank"){
                ...
                //Log::info($store);
                $wbStore = WeBankStoreUnion::where('store_id',$store->store_id)->where('payment_type','24')->first();
                $orderId = 'b'.date('YmdHis', time()).mt_rand(100000,999999);
...
//                    'body' => $request->get('desc') . '门店收款信息',
                ...
                $result= $wbapp->tradepay(2,$orderId,$data['code'],$data['price'],$request->get('desc') . '机具收款',$request->get('desc'),$m_id,$store->store_id);
                Log::info($result);
                return $result;
//                //self::Barcodepay($orderId,$wbStore->wb_merchant_id,$data['code'],$data['price'],$request->get('desc') . '门店收款',$request);
//
//                Log::info($wbStore);
...
//                ]);
        ...
        if ($str == "13") {
            ...
            $msg = '尚未支持'. $MerchantPayWay->weixin .'通道';
            return json_encode([
                'status' => 0,
                'msg' => $msg,
            ]);
    ...
    public function Barcodepay($orderId,$wbMerchantId,$authCode,$totalAmount,$subject,$other=null){
