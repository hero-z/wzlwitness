...
class NewOrderManageController extends AlipayOpenController
{
    ...
    protected $qrcodelists=[101=>'当面付',102=>'口碑',104=>'当面付固定金额',106=>'口碑固定金额',201=>'微信',203=>'微信固定金额',301=>'平安支付宝',302=>'平安微信',303=>'平安京东',304=>'平安翼支付',401=>'银联固定金额',501=>'民生支付宝',502=>'民生微信',503=>'民生QQ钱包',601=>'浦发支付宝',602=>'浦发微信','801'=>'微众支付宝','802'=>'微众微信','803'=>'微众支付宝机具','804'=>'微众微信机具'];
    protected $shopTable=[1=>'alipay_app_oauth_users',2=>'alipay_shop_lists',3=>'weixin_shop_lists',4=>'pingan_stores',5=>'pufa_stores',6=>'union_pay_stores',7=>'ms_stores',8=>'we_bank_stores'];  //微众银行 modified by Neomor
...
//        $request->session()->put('adminlist',$list);
            ...
            foreach($shoplists as $k=>$v){
                switch(substr($v->store_id,0,1)){
                    ...
                    case 'm':
                        ...
                        break;
                    case 'b':  //微众银行 add by Neomor
                        $shoplists[$k]->store_name=$v->store_name.'(微众银行)';  //微众银行 add by Neomor
                        break;  //微众银行 add by Neomor
                    default:
    ...
    public function expexceldata(Request $request){
        try{
            ...
            $head=['订单号','店铺名','金额','状态','支付类型','员工','备注','更新时间'];
            $body=[$head];
            $username='店铺收款';
            $store_name='';
            ...
            foreach($list as $k=>$v){
                ...
                $body[]=[$v->out_trade_no." ",$store_name." ",$v->total_amount,$statusstr.'  ',$paystr,$username,$v->remark,$v->updated_at];
            }
            $cellData = $body;
...
//            \PHPExcel_Settings::setCacheStorageMethod($cacheMethod,$cacheSettings);
    ...
    public function getdplists($id){
        ...
        try{
            ...
            //微众银行 add by Neomor
            $res[]=DB::table('we_bank_stores')  //微众银行 add by Neomor
                ->when($id, function ($query) use ($id) {  //微众银行 add by Neomor
                    return $query->where('user_id', $id);  //微众银行 add by Neomor
                })  //微众银行 add by Neomor
                ->select('store_id','alias_name as store_name','user_id');  //微众银行 add by Neomor
