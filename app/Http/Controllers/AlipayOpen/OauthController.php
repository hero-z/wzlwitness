...
//                return json_encode(['code' => 200, 'msg' => "添加成功"]);
    ...
    public function oauthlist(Request $request)
    {
        ...
        //
        $data = DB::table('users')->where('alipay_app_oauth_users.pid', 0)
            ->where($where)->select('users.name', 'alipay_app_oauth_users.*')
            ->orderBy('updated_at', 'desc')
            ->where('promoter_id', Auth::user()->id)
            ->join('alipay_app_oauth_users', 'alipay_app_oauth_users.promoter_id', '=', 'users.id')
            ->where("alipay_app_oauth_users.is_delete", 0)->get();
