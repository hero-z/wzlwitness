...
//
            ...
            //微众银行          //add by Neomor
            //微众总店铺数 //add by Neomor
            $wbstores = DB::table('we_bank_stores')->count(); //add by Neomor
            //微众昨日店铺
            $wbstore_y = DB::table('we_bank_stores') //add by Neomor
                ->where('updated_at', '>', date('Y-m-d' . ' ' . ' 00:00:00', strtotime('-1 day')))          //add by Neomor
                ->where('updated_at', '<', date('Y-m-d' . '  ' . ' 23:59:59', strtotime('-1 day')))          //add by Neomor
                ->count();          //add by Neomor
            //微众总交易量
            $wbtotal_amount = Order::whereBetween('type',[801,804]) //add by Neomor
                ->where('pay_status',1)          //add by Neomor
                ->sum('total_amount');          //add by Neomor
            //微众昨天流水          //add by Neomor
            $wbtotal_y  = Order::whereBetween('type',[801,804]) //add by Neomor
                ->where('pay_status',1)          //add by Neomor
                ->where('updated_at', '>', date('Y-m-d' . ' ' . ' 00:00:00', strtotime('-1 day')))          //add by Neomor
                ->where('updated_at', '<', date('Y-m-d' . '  ' . ' 23:59:59', strtotime('-1 day')))          //add by Neomor
                ->sum('total_amount');          //add by Neomor
            ...
            //总交易量
            $ttotal_amount = $atotal_amount+$wtotal_amount+$ptotal_amount+$ftotal_amount+$wbtotal_amount;         //modified by Neomor
            //总店铺数
            $tstores = $astores+$wstores+$pstores+$fstores+$wbstores;         //modified by Neomor
            //昨日店铺
            $tstore_y = $astore_y+$wstore_y+$pstore_y+$fstore_y+$wbstore_y;         //modified by Neomor
            //昨天流水
            $ttotal_y = $atotal_y+$wtotal_y+$ptotal_y+$ftotal_y+$wbtotal_y;         //modified by Neomor
        }
        else {
            //支付宝
...
//
            ...
            //微众银行          //add by Neomor
            //微众总交易量          //add by Neomor
            $wbtotal_amount =DB::table('orders')          //add by Neomor
                ->join('we_bank_stores','we_bank_stores.store_id','orders.store_id')          //add by Neomor
                ->where('we_bank_stores.user_id',Auth::user()->id)          //add by Neomor
                ->whereBetween('orders.type',[801,804])          //add by Neomor
                ->where('orders.pay_status',1)          //add by Neomor
                ->sum('orders.total_amount');          //add by Neomor
            //微众总店铺数
            $wbstores = DB::table('we_bank_stores')->where('user_id',Auth::user()->id)->count();          //add by Neomor
            //微众昨日店铺
            $wbstore_y = DB::table('we_bank_stores')          //add by Neomor
                ->where('user_id',Auth::user()->id)          //add by Neomor
                ->where('updated_at', '>', date('Y-m-d' . ' ' . ' 00:00:00', strtotime('-1 day')))          //add by Neomor
                ->where('updated_at', '<', date('Y-m-d' . '  ' . ' 23:59:59', strtotime('-1 day')))          //add by Neomor
                ->count();          //add by Neomor
            //微众昨天流水
            $wbtotal_y  = DB::table('orders')         //modified by Neomor
                ->join('we_bank_stores','we_bank_stores.store_id','orders.store_id')
                ->where('we_bank_stores.user_id',Auth::user()->id)
                ->whereBetween('orders.type',[801,804])
                ->where('orders.pay_status',1)
                ->where('orders.updated_at', '>', date('Y-m-d' . ' ' . ' 00:00:00', strtotime('-1 day')))
                ->where('orders.updated_at', '<', date('Y-m-d' . '  ' . ' 23:59:59', strtotime('-1 day')))
                ->sum('orders.total_amount');
        ...
        return view('admin.alipayopen.home', compact('data', 'ttotal_amount', 'tstores','ttotal_y','tstore_y', 'atotal_amount', 'astores','atotal_y','astore_y', 'wtotal_amount', 'wstores','wtotal_y','wstore_y', 'ptotal_amount', 'pstores','ptotal_y','pstore_y','ftotal_amount', 'fstores','ftotal_y','fstore_y','wbstores','wbstore_y','wbtotal_amount','wbtotal_y'));
    }
}
