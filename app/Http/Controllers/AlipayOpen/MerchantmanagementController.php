...
use App\Http\Controllers\Merchant\PayWaysController;
    ...
    public function mmdatalists(Request $request){
        ...
        if(Auth::user()->hasRole('admin')){
            ...
            $res[]=DB::table('weixin_shop_lists')
                ...
                ->where($wheresql)
                ->select('merchant_shops.merchant_id');
            $res[]=DB::table('we_bank_stores')                    //add by Neomor 支持微众银行
                ->join('users','users.id','we_bank_stores.user_id')                   //add by Neomor 支持微众银行
                ->join('merchant_shops','merchant_shops.store_id','we_bank_stores.store_id')                   //add by Neomor 支持微众银行
                ->where($wheresql)                   //add by Neomor 支持微众银行
                ->select('merchant_shops.merchant_id');                   //add by Neomor 支持微众银行
...
//            dd($list);
    ...
    public function mmshopbind(Request $request)
    {
        ...
        return view('admin.alipayopen.merchantshopbind', compact('first','second','third','four','five','six','seven','eight',"nine",'name','id'));
    }
    public function mmpostdata(Request $request)
    {
        ...
        $type=['oalipay'=>'支付宝当面付','salipay'=>'口碑门店收款','pingan'=>'平安通道','weixin'=>'官方微信支付','pufa'=>'浦发通道','unionpay'=>'银联通道','ms'=>'民生通道','webank'=>'微众通道','newland'=>"新大陆通道"];
        $insertdata=[];
...
//添加收银通道
    public function mmshopsetway(Request $request)
    {
        $auth = Auth::user()->can('bindShops');
        if (!$auth) {
            echo '你没有权限操作！';
...
//            dd($list);
        ...
        $m="";
        try {
            $merchant = MerchantShops::where('merchant_id', $id)->get();//商户信息
            if ($merchant) {
        ...
        try {
            $m = MerchantPayWay::where('merchant_id', $id)->get();
            if ($m) {
                $m = $m->toArray();
            }
        } catch (\Exception $exception)
...
//
        Log::info($merchant);
//        Log::info($m);
        return view('admin.setWays.set', compact('merchant','m'));
//        if ($list){
//            $data = $list->toArray();
//            //非数据库模型自定义分页
//            $perPage = 9;//每页数量
...
//            $paginator='';
//        }
//        return view('admin.alipayopen.paywayindex', ['data'=>$datapage,'paginator'=>$paginator,'id'=>$id]);
//        //return view('admin.alipayopen.paywayindex');
    }
    //设置通道提交
    public function mmshopsetwaypost(Request $request)
    {
...
//        $data['merchant_id'] = auth()->guard('merchant')->user()->id;
    ...
    public static function adminSetWays($m_id)
    {
        //$m_id = auth()->guard('merchant')->user()->id;


    }
    public function mmshopaddpost(Request $request)
    {
        return view('admin.alipayopen.paywayadd');
    }

    public function mmshopwaymgt(Request $request)
    {
        dd($request);
    }
}
