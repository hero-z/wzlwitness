...
class ticketController extends Controller
{
    ...
    public function addUprint(Request $request){
        ...
        $unionpay=DB::table('union_pay_stores')
            ...
            ->get();
        $webank=DB::table('we_bank_stores')   //add by Neomor 支持微众银行
            ->where('user_id',Auth::user()->id)    //add by Neomor 支持微众银行
            ->where("is_delete",0)    //add by Neomor 支持微众银行
            ->select('store_id',"alias_name as name")    //add by Neomor 支持微众银行
            ->get();   //add by Neomor 支持微众银行
        ...
        return view("admin.ticket.addUprint", compact("oali", "sali", "weixin", "pingan","pufa","unionpay","webank")); //modify by Neomor
    }
    //添加u印
   ...
   public function updateUprint(Request  $request){
    ...
    public function addMerchine()
    {
        ...
        $unionpay=DB::table('union_pay_stores')
            ...
            ->select('store_id',"alias_name as name")
            ->get();
        $webank=DB::table('we_bank_stores')  //add by Neomor
            ->where('user_id',Auth::user()->id)  //add by Neomor
            ->where("is_delete",0)  //add by Neomor
            ->select('store_id',"alias_name as name")  //add by Neomor
            ->get();  //add by Neomor
        if (Auth::user()->hasRole('admin')) {
            //支付宝当面付店铺
            $oali = DB::table('alipay_app_oauth_users')
        ...
        return view("admin.ticket.addMerchine", compact("oali", "sali", "weixin", "pingan","pufa","unionpay","webank"));
    }
