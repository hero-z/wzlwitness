...
class alipassController extends Controller
{
    ...
    {
        $list=DB::table("alipay_passes")
            ->join("alipay_app_oauth_users","alipay_passes.store_id","=","alipay_app_oauth_users.store_id")
            ->select("alipay_passes.*","alipay_app_oauth_users.auth_shop_name")
            ->paginate(8);
    ...
    public function createAlipass(Request $request)
    {
        ...
        if (!empty($resultCode) && $resultCode == 10000) {
            ...
            DB::table("alipay_passes")->insert($data);
            return json_encode([
                "success"=>"1"
            ]);
        } else {
    ...
    //使用卡券
    public function  updatecardTemplete($serial_number,$channel_id,$user_id){
        $ao = new AlipayOpenController();
        $aop = $ao->AopClient();
        $aop->method = "alipay.pass.instance.update";
        $requests = new AlipayPassInstanceUpdateRequest ();
