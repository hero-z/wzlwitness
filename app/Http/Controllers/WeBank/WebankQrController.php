...
class WebankQrController  extends WebankBaseController
{
...
//        }
        try {
            ...
            if($info){
                ...
                if ($pay_type == "other") { //非空码，必须是支付宝或者微信扫码
                   die('请用支付宝或者微信扫描二维码');
                }
                ...
                if ($info->code_type == 1) {
                    ...
                    if ($pay_type == "alipay") {
                        ...
                        $code_url = $config['app_auth_url'] . '?app_id=' . $config['app_id'] . "&redirect_uri=" .
                                    $config['callback'] . '&scope=auth_base&state=WB_' . $store_id.'_'.$merchant_id;
                        Log::info($code_url);
                    ...
                    if ($pay_type == "weixin") {
                        $store_id = $store->store_id;
                        $code_url = url('admin/weixin/oauth?scope=snsapi_userinfo&sub_info=WB_' . $store_id.'_'.$merchant_id);
                        return redirect($code_url);
                    }
                }
            }else{
