...
class WebankStoreController  extends WebankBaseController
{
    ...
    const MERCHANT_ADDSUBDEV_PATH = '/api/aap/server/wepay/addsubdevconfig';
    const MERCHANT_QUERYSUBDEV_PATH='/api/aap/server/wepay/querysubdevconfig';
    ...
    public function register(Request $request){
        ...
        $store_id= WebankAopClient::getStoreId('b');
        $info='出错了';
        //判断是不是微信
        $agent_type='other';
        ...
        try{
            ...
            if ($wxres['code'] == 0&&$wxres['success']||$alires['code'] == 0&&$alires['success']) {
                ...
                DB::table('we_bank_storeunion')->insert([
                    ...
                    'settlement_type'=>'11',
                    'code_no'=>'2023640000',   //Modified by Neomor 记录开户时候的代理商号
                ]);
                DB::table('we_bank_storeunion')->insert([
                    'store_id'=>$store_id,
                    'product_type'=>'003',
                    ...
                    'settlement_type'=>'01',
                    'code_no'=>'2033640001',  //Modified by Neomor 记录开户时候的代理商号
                ]);
...
//        $postdata['district']='0755';//对应地区码表
    ...
    public function editSubdevConfig(Request $request){
        $store_id=$request->store_id;
        $store=WeBankStore::where('store_id',$store_id)->where('is_delete',0)->first();
        $jsapiPath=null;
        $sub_appid=null;
        $subscribe_appid=null;
        if($store){
            $wxstore_union=DB::table('we_bank_storeunion')->where('store_id',$store_id)->where('product_type','004')->first();
            if($wxstore_union&&$wxstore_union->wb_merchant_id){

                $result = $this->querySubdevConfig($wxstore_union->wb_merchant_id,$wxstore_union->code_no,1);
                if ($result['code']=='0'){
                    $jsapiPaths= json_decode($result['jsapiPathList'],true);
                    $jsapiPath=$jsapiPaths['jsapi_path_list']['0'];
                    $appidConfigList = json_decode($result['appidConfigList'],true);
                    $appconfig =$appidConfigList['appid_config_list']['1']??null;
                    $sub_appid=$appconfig['sub_appid'];
                    $subscribe_appid=$appconfig['subscribe_appid'];
                }
                //Log::info($result);

                $subdevconfig=(object)$result;
            }else{
                $subdevconfig=9;
            }

            return view('admin.webank.eidtsubdevconfig',compact('store','wxstore_union','jsapiPath','sub_appid','subscribe_appid','subdevconfig'));
        }else{
            return back();
        }
    }

    public function editsubdevconfigpost(Request $request)
    {
        $merchantid = $request->wx_wb_merchant_id;
        $storeUnion = WeBankStoreUnion::where('wb_merchant_id',$merchantid)->first();
        $code_no = $storeUnion->code_no;
        $result1 = self::addSubdev($merchantid,$code_no,'jsapiPath',$request->jsapi_path_list);
        $result2 = self::addSubdev($merchantid,$code_no,'subAppid',$request->sub_appid);
        $result3 = self::addSubdev($merchantid,$code_no,'subscribeAppid',$request->subscribe_appid);
    ...
    public function addSubdev($wb_merchant_id,$code_no,$paramName,$paramValue)
    {
        $webank=$this->WebankHelper(1);
        $app_id = $webank->appId;
        $version = $webank->version;
        $nonce = $webank->getNonce();
        $data=[
        ...
        $header = ['Content-Type: application/json'];
        $request = array(
            'url' => $webank->headUrl.self::MERCHANT_ADDSUBDEV_PATH. $url_params,
            'method' => 'post',
            'timeout' => self::$timeout,
            'data' => $jsonData,
            'header' => $header,
...
//        Log::info($result);
    ...
    public function querySubdevConfig($wb_merchant_id,$code_no,$id){
        $webank=$this->WebankHelper($id);
        $app_id = $webank->appId;
        $version = $webank->version;
        $nonce = $webank->getNonce();
        ...
        $header = ['Content-Type: application/json'];
        $request = array(
            'url' => $webank->headUrl.self::MERCHANT_QUERYSUBDEV_PATH . $url_params,
            'method' => 'post',
            'timeout' => self::$timeout,
            'data' => $jsonData,
            'header' => $header,
