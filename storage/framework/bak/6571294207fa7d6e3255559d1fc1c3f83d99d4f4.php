...
<?php $__env->startSection('content'); ?>
    <body ontouchstart>
    <input type="hidden" id="store_id" value="<?php echo e($shop->store_id); ?>">
    <input type="hidden" id="m_id" value="<?php echo e($m_id); ?>">
    <div class="weui-wepay-pay__ft">
        <p class="weui-wepay-pay__info" style="font-size: 18px;"><?php echo e($shop->alias_name); ?></p>
    </div>
    ...
    <script>
        function callpay() {
            $.post("<?php echo e(route('webankwxdoPay')); ?>", {
                            ...
                            WeixinJSBridge.invoke(
                                    ...
                                    function (res) {
                                        if (res.err_msg == "get_brand_wcpay_request:ok") {
                                            window.location.href = "<?php echo e(url('admin/webank/weixin/wxpaysuccess?price=')); ?>"+$("#total_fee").val();
                                            // 使用以上方式判断前端返回,微信团队郑重提示：
                                            // res.err_msg将在用户支付成功后返回 ok，但并不保证它绝对可靠。
                                        }else{
                                            window.location.href = "<?php echo e(route('webankwxpayerror',['code'=>'111'])); ?>";
                                        }
                                    }
                            );
                        } else {
                            window.location.href = "<?php echo e(route('webankwxpayerror')); ?>";
                        }
                    }, 'json');
        }
    </script>
