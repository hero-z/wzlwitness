...
$api->version('v1', function ($api) {
   ...
   // 接口需要token验证 商户端接口
    ...
    $api->group(['namespace' => 'App\Api\Controllers\Merchant', 'prefix' => 'merchant','middleware' => [/*'jwt.auth', 'jwt.refresh','auth:merchantApi'*/'merchant.api']], function ($api) {
        ...
        $api->any('Reverse','MerchantController@Reverse')->name('ReverseApi');//add by Neomor 撤单接口API
    ...
    // 接口需要token验证 商户端接口     //add by Neomor 自己修改的接口
    $api->group(['namespace' => 'App\Api\Controllers\Merchant', 'prefix' => 'cashier','middleware' => [/*'jwt.auth', 'jwt.refresh','auth:merchantApi'*/'merchant.api']], function ($api) {
        $api->any('MOrder', 'CashierController@MOrder');
        $api->any('TradePay', 'CashierController@TradePay')->name('TradePayApi');
        $api->any('MoneyPay', 'CashierController@MoneyPay');
        $api->any('TradeQuery', 'CashierController@TradeQuery');
        $api->any('Order', 'CashierController@Order');
//        $api->any('Questions', 'QuestionController@getQuestions');
//        $api->any('add', 'InsertController@addMerchant');
//        $api->any('store', 'SelectController@selectStore');
//        $api->any('pay_qr_url', 'SelectController@selectPayQr');
